...
import com.midea.cloud.srm.model.bid.purchaser.projectmanagement.bidingresult.entity.BidResult;
import com.midea.cloud.srm.model.bid.purchaser.projectmanagement.bidingresult.param.BidResultParam;
...
import com.midea.cloud.srm.model.bid.purchaser.projectmanagement.techproposal.vo.AbandonmentBidSupVO;
...
public class BidingResultController extends BaseController {
    ...
    public List<Map<String, Object>> getTemplateReport(@RequestBody EvaluationResult result) {
        ...
        results = bidOrderLineTemplateReportLineVOS.stream().map(e -> {
            ...
            e.getPrice().forEach(q -> {
                ...
                row.put(q.getVendorName(), q.getTaxPrice());
            });
            return row;
    ...
    public List<EvaluationResult> calculateQuota(@RequestParam Long biddingId) {
        List<EvaluationResult> evaluationResults = evaluationService.calculateQuotaResult(biddingId);
