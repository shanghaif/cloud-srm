...
public class BidSignUpController extends BaseController {
    ...
    public BidSignUpVO signUpInfo(@RequestBody BidSignUpVO bidSignUpVO) {
        ...
        for(BidVendorFileVO bidVendorFile : vendorFileVOs) {
            BidFileConfig bidFileConfig =bidFileConfigService.getById(bidVendorFile.getRequireId());
            if(bidFileConfig != null){
                bidVendorFile.setDocId(bidFileConfig.getReferenceFileId());
                bidVendorFile.setReqFileName(bidFileConfig.getReferenceFileName());
            }
