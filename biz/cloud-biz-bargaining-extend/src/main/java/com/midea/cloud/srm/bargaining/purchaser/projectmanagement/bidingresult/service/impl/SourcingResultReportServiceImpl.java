...
import com.midea.cloud.srm.feign.inq.SourcingResultReportClient;
...
public class SourcingResultReportServiceImpl implements ISourcingResultReportService {
    ...
    @Resource
    private SourcingResultReportClient sourcingResultReportClient;
    @Resource
    ...
    private final static String key = "lock-generateReport-brg";
    ...
    public SourcingResultReport generate(String biddingNum, Long inquiryId) {
        ...
        if (!isLock) {
            ...
            throw new BaseException(String.format("亲爱的%s,您好，此单正生成寻源结果报表，请稍后再刷新查看^_^!",Objects.isNull(loginAppUser)?"用户":loginAppUser.getNickname()));
        }
        try {
