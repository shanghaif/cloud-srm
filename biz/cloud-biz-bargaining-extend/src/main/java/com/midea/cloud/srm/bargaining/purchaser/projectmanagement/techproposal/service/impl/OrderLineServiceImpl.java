...
import com.midea.cloud.srm.model.bid.purchaser.projectmanagement.enums.PaymentDayEnum;
...
// 获取 付款信息
        ...
        return orderLines.stream()
                .map(line -> {
                    ...
                    BidOrderLineVO vo = new BidOrderLineVO();
                    BeanUtils.copyProperties(line, vo);
                    vo.setPaymentTermList(collect.get(line.getOrderLineId()));
                    return vo;
                })
                .collect(Collectors.toList());
    }
    ...
    public List<BidOrderLineVO> getOrderLineByBidingIdAndVendorId(Long bidingId, Long vendorId) {
        ...
        orderLineVOS = judgeShowTargetPrice(bidingId, orderLineVOS,vendorId);
        return orderLineVOS;
    }
     ...
     * @return
     */
    private List<BidOrderLineVO> judgeShowTargetPrice(Long bidingId, List<BidOrderLineVO> orderLineVOS,Long vendorId) {
            ...
            if (paymentTermList != null) {
                for (OrderlinePaymentTerm relate : paymentTermList) {
                    ...
                    if (Objects.isNull(relate.getPaymentDay())) {
                        if (relate.getPaymentDayCode().equals("00")) {
    ...
    protected void validateOrderLine(BidOrderLineVO orderLineVO, boolean isModel, boolean isInPrice) {
        ...
        if ((orderLineVO.getTaxRate() == null || orderLineVO.getTaxKey() == null)) {
            if (!isModel) {
        ...
        if (Objects.isNull(orderLineVO.getPrice()) || orderLineVO.getPrice().compareTo(BigDecimal.ZERO) <= 0) {
            throw new BaseException(String.format("物料%s的供应商报价要大于0", orderLineVO.getTargetDesc()));
        }
