...
//		supcooperateClient.saveOrUpdateReturnOrder(saveOrUpdateReturnOrder);
	...
	private List<OrderDetail> deductionReceiveSum(Map<Long, BigDecimal> decutionMap) {
		List<OrderDetail> orderDetails = supcooperateClient.getOrderDetailListByIds(Arrays.asList(decutionMap.keySet().toArray(new Long[] {})));
	...
	private List<DeliveryNotice> openDeliveryNotice(List<Long> deliveryNoteDetailIds) {
		...
		List<DeliveryNotice> deliveryNotices = supcooperateClient.queryDeliveryNoticeByIds(deliveryNoticeIds);
	...
	public void batchConfirm(List<Long> returnOrderIds) {
		List<ReturnDetail> returnDetails = supcooperateClient.queryReturnDetailByReturnOrderIds(returnOrderIds);
		List<Long> deliveryNoteDetailIds = returnDetails.stream().map(ReturnDetail::getDeliveryNoteDetailId).collect(Collectors.toList());
		List<DeliveryNoteDetail> deliveryNoteDetails = supcooperateClient.queryDeliveryNoteDetailByIds(deliveryNoteDetailIds);
		List<Long> orderDetailIds = deliveryNoteDetails.stream().map(DeliveryNoteDetail::getOrderDetailId).collect(Collectors.toList());
		List<OrderDetail> orderDetails = supcooperateClient.getOrderDetailListByIds(orderDetailIds);
		...
		List<ReturnOrder> returnOrderByIds = supcooperateClient.getReturnOrderByIds(returnOrderIds);
		...
		supcooperateClient.confirmReturnOrders(confirmReturnOrders);
	}
}
