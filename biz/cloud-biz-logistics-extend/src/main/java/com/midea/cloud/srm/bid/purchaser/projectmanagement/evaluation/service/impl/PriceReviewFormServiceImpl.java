...
import com.midea.cloud.srm.bid.projectlist.mapper.BidVendorFileMapper;
...
//
//        if ("Y".equals(bidding.getGeneratePurchaseApproval()))
//            throw new BaseException("物流单: [" + bidding.getBidingNum() + "]已生采购订单。");
//
//        if (BiddingProjectStatus.PROJECT_END.equals(BiddingProjectStatus.get(bidding.getBidingStatus())))
//            throw new BaseException("物流单: [" + bidding.getBidingNum() + "]非已结项状态，不能生成采购订单审批。");
//
//
//        // 获取 中标结果
...
//            }
//            if (isNullQuota) {
//                BigDecimal quantity = Optional.ofNullable(winningResult.getQuotaQuantity()).orElseThrow(() -> new BaseException("请维护中标数量"));
//                //每一中标行的中标数量*含税单价
...
//                .sourceType(SourcingType.TENDER.getItemValue())
//                .ceeaTitle(bidding.getBidingName())
//                .businessTitle(bidding.getBidingName())
//                .ceeaSourceNo(bidding.getBidingNum())
//                .ceeaAwareWay(bidding.getBidingAwardWay())
//                .ceeaAllocationType(bidding.getQuotaDistributeType())
