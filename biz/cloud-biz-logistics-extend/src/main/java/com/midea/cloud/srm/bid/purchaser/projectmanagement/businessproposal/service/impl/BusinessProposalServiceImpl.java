...
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.businessproposal.vo.BusinessItemVO;
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.businessproposal.vo.CancelBidParam;
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.businessproposal.vo.OrderDetailVO;
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.enums.BidingFileTypeEnum;
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.techproposal.entity.OrderHead;
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.techproposal.entity.OrderHeadFile;
        ...
        for (Long requirementLineId : firstRoundOrderLineMap.keySet()) {
            ...
            for (int round = BidConstant.FIRST_ROUND; round <= biding.getCurrentRound(); round++) {
                ...
                for (Long bidVendorId : bidVendorIdSet) {
                    dataItem.add(priceMap.get(bidVendorId) == null ? null : priceMap.get(bidVendorId).getPrice()
//                            .setScale(biding.getpricePrecision(), RoundingMode.HALF_DOWN)
                            .toPlainString());
                }
                dataItem.add(list.get(0).getCurrentRoundMinPrice() == null ? null : list.get(0).getCurrentRoundMinPrice()
//                        .setScale(biding.getDecimalAccuracy(), RoundingMode.HALF_DOWN)
                        .toPlainString())
                ;
                dataItem.add(line.getTargetPrice() == null ? null : line.getTargetPrice()
//                        .setScale(biding.getDecimalAccuracy(), RoundingMode.HALF_DOWN)
                        .toPlainString());
            }
            String winBidPrice = "";
            ...
            if (winBidOrderLineList != null) {
                for (OrderLine o : winBidOrderLineList) {
                    ...
                    winBidPrice = winBidPrice + o.getPrice()
//                            .setScale(biding.getDecimalAccuracy(), RoundingMode.HALF_DOWN)
                            .toPlainString() + ",";
