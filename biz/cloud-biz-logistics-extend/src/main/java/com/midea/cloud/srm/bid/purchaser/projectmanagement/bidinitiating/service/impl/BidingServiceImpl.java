...
import com.midea.cloud.srm.model.logistics.bid.purchaser.projectmanagement.bidinitiating.vo.ProjectInfoVO;
...
public class BidingServiceImpl extends ServiceImpl<BidingMapper, Biding> implements IBidingService {
    ...
    public PageInfo<Biding> listPage(Biding biding) {
        ...
        LambdaUpdateWrapper<Biding> wrapper = Wrappers.lambdaUpdate(Biding.class).
                like(StringUtil.notEmpty(biding.getProjectCode()), Biding::getProjectCode, biding.getProjectCode()).
                like(StringUtil.notEmpty(biding.getProjectName()), Biding::getProjectName, biding.getProjectName()).
                in(StringUtil.notEmpty(biding.getBusinessModeCode()), Biding::getBusinessModeCode, biding.getBusinessModeCode()).
                in(StringUtil.notEmpty(biding.getTransportModeCode()), Biding::getTransportModeCode, biding.getTransportModeCode()).
                in(StringUtil.notEmpty(biding.getBusinessType()), Biding::getBusinessType, biding.getBusinessType()).
                in(StringUtil.notEmpty(biding.getCurrentRound()), Biding::getCurrentRound, biding.getCurrentRound()).
                like(StringUtil.notEmpty(biding.getApplicationNum()), Biding::getApplicationNum, biding.getApplicationNum()).
                like(StringUtil.notEmpty(biding.getCreatedByName()), Biding::getCreatedByName, biding.getCreatedByName()).
                in(StringUtil.notEmpty(biding.getBidingStatus()), Biding::getBidingStatus, biding.getBidingStatus());
        ...
        if (CollectionUtils.isNotEmpty(requirementLineList)) {
            ...
            requirementLineList.forEach(requirementLine -> {
                ...
                bidLine.setRequirementLineId(IdGenrator.generate()).setRequirementId(requirementId).
                        ...
                        setOrgName(requirementLine.getOrgName()).setCategoryId(requirementLine.getCategoryId()).
...
//        }
        ...
        if (biding.getEnrollEndDatetime().compareTo(new Date()) <= 0) {
