...
public class FsscStatusServiceImpl extends ServiceImpl<FsscStatusMapper, FsscStatus> implements IFsscStatusService {
    ...
    private void updatePurchaseBoeLgi(FsscStatus fsscStatus) {
        ...
        if (FsscStatusCode.FSSC_STATUS_10.getValue().equals(status)) {//单据撤回,对应改变网上开票为新建
            scIOnlineInvoiceService.statusReturn(fsscNo, InvoiceStatus.DRAFT.name());
        } else if (FsscStatusCode.FSSC_STATUS_11.getValue().equals(status)) {//单据驳回,对应改变网上开票为已驳回
            List<OnlineInvoiceAdvance> onlineInvoiceAdvances = scIOnlineInvoiceAdvanceService.listOnlineInvoiceAdvanceByFsscNo(fsscNo);
            if (CollectionUtils.isNotEmpty(onlineInvoiceAdvances)) {
                ...
                scIOnlineInvoiceAdvanceService.updateOnlineInvoiceAdvanceByParam(onlineInvoiceAdvances);
            }
            scIOnlineInvoiceService.statusReturn(fsscNo, InvoiceStatus.REJECTED.name());
...
//                }
//            }
            scIOnlineInvoiceService.statusReturn(fsscNo, InvoiceStatus.REVIEWED.name());
            log.info("----------------------------------已完成三单匹配审批同意------------------------------");
        } else if (FsscStatusCode.FSSC_STATUS_100.getValue().equals(status)) {
            scIOnlineInvoiceService.importStatusReturn(fsscNo, InvoiceImportStatus.IMPORTED.name());
            log.info("----------------------------------已完成三单匹配导入----------------------------------");
        }
    }
