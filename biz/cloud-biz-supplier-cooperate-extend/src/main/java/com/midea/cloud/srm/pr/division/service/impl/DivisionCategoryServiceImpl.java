...
public class DivisionCategoryServiceImpl extends ServiceImpl<DivisionCategoryMapper, DivisionCategory> implements IDivisionCategoryService {
    ...
    @Override
    public PageInfo<DivisionCategory> queryDivisionCategory(DivisionCategory divisionCategory) {
        PageUtil.startPage(divisionCategory.getPageNum(),divisionCategory.getPageSize());
        List<DivisionCategory> divisionCategories = this.list(Wrappers.lambdaQuery(DivisionCategory.class).
                like(StringUtil.notEmpty(divisionCategory.getPersonInChargeUsername()), DivisionCategory::getPersonInChargeUsername, divisionCategory.getPersonInChargeUsername()).
                like(StringUtil.notEmpty(divisionCategory.getPersonInChargeNickname()), DivisionCategory::getPersonInChargeNickname, divisionCategory.getPersonInChargeNickname()).
                and(wrapper -> {
                    wrapper.isNull(DivisionCategory::getEndDate).or().
                            ge(DivisionCategory::getEndDate, new Date());
                }).groupBy(DivisionCategory::getPersonInChargeUsername)
        );
        return new PageInfo<>(divisionCategories);
    }

    @Override
    public List<Long> queryCategoryIdByUserId(Long userId) {
        List<Long> categoryIds = new ArrayList<>();
        if (!ObjectUtils.isEmpty(userId)) {
            List<DivisionCategory> categories = this.list(Wrappers.lambdaQuery(DivisionCategory.class).eq(DivisionCategory::getPersonInChargeUserId, userId));
            if(org.apache.commons.collections4.CollectionUtils.isNotEmpty(categories)){
                categoryIds = categories.stream().map(DivisionCategory::getCategoryId).collect(Collectors.toList());
            }
        }
        return categoryIds;
    }
    ...
    public void saveOrUpdateDivisionCategory(List<DivisionCategory> divisionCategories) {
        ...
        if (!CollectionUtils.isEmpty(divisionCategories)) {
            /**
             * 增加校验:业务实体+库存组织+物料小类+负责人（工号）+职责，作为数据唯一性标识。
             */
            divisionCategories.forEach(divisionCategory -> {
                if(ObjectUtils.isEmpty(divisionCategory.getDivisionCategoryId())){
                    ...
                    Assert.isTrue(CollectionUtils.isEmpty(list),"业务实体+库存组织+物料小类+负责人（工号）+职责,存在重复");
                }else {
                    List<DivisionCategory> list = this.list(Wrappers.lambdaQuery(DivisionCategory.class).
                            eq(DivisionCategory::getOrgId, divisionCategory.getOrgId())
                            .eq(DivisionCategory::getOrganizationId, divisionCategory.getOrganizationId())
...
//        }
    ...
    public PageInfo<DivisionCategory> listPageByParam(DivisionCategoryQueryDTO divisionCategoryQueryDTO) {
        ...
        String erpNum = divisionCategoryQueryDTO.getErpNum();
        if (StringUtils.isNotBlank(erpNum)) {
            Map<String, User> userByNos = rbacClient.getUserByNos(Collections.singletonList(erpNum));
            if (Objects.nonNull(userByNos.get(erpNum))) {
                Long userId = userByNos.get(erpNum).getUserId();
                if (Objects.nonNull(userId)) {
                    queryWrapper.eq("PERSON_IN_CHARGE_USER_ID", userId);
                }
            }
        }
        ...
        List<DivisionCategory> result = this.list(queryWrapper);
        Set<Long> collect = result.stream().map(DivisionCategory::getPersonInChargeUserId).collect(Collectors.toSet());
        Map<Long, User> userMap = rbacClient.getByUserIds(collect).stream().collect(Collectors.toMap(User::getUserId, Function.identity()));
        result.forEach(e -> e.setErpNum(userMap.get(e.getPersonInChargeUserId()).getCeeaEmpNo()));
        ...
        Function<Collection<DivisionCategoryModelDTO>, Collection<DivisionCategory>> operator = (e -> {
            ...
            for (DivisionCategoryModelDTO current : e) {
                boolean error = false;
                {
                    ...
                    //kuangzm 判断字段  是否主要负责人(Y/N) 值是否正确
                    if(!Objects.equals(current.getIfMainPerson(), YesOrNo.YES.getValue()) && !Objects.equals(current.getIfMainPerson(), YesOrNo.NO.getValue())) {
                    	 errMsg.append("是否主要负责人(Y/N),请输入Y/N,");
                         error = true;
                    }
    ...
    public List<List<Object>> queryExportData(ExportExcelParam<DivisionCategory> param) {
        ...
        List<DivisionCategory> list = list(Wrappers.lambdaQuery(queryParam)
                .orderByDesc(DivisionCategory::getLastUpdateDate));
        Set<Long> collect = list.stream().map(DivisionCategory::getPersonInChargeUserId).collect(Collectors.toSet());
        Map<Long, User> userMap = rbacClient.getByUserIds(collect).stream().collect(Collectors.toMap(User::getUserId, Function.identity()));
        List<DivisionCategoryModelDTO> categoryDvs = list
                .stream().map(e -> {
                    ...
                    String ceeaEmpNo = userMap.get(e.getPersonInChargeUserId()).getCeeaEmpNo();
                    divisionCategoryModelDTO.setErpNum(ceeaEmpNo);
        ...
        if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(categoryDvs)) {
            ...
            Field[] fields = DivisionCategoryModelDTO.class.getDeclaredFields();
            for (DivisionCategoryModelDTO categoryDv : categoryDvs) {
                Map<String, Object> entityMap = new HashMap<>();
                for (Field field : fields) {
                    field.setAccessible(true);
                    if (field.isAnnotationPresent(ExcelProperty.class)) {
                        try {
                            entityMap.put(field.getName(), field.get(categoryDv));
                ...
                for (Map<String, Object> map : mapList) {
                    ...
                    for (String key : titleList) {
                        ...
                        if (Objects.equals("duty", key)) {
                            ...
                            objects.add(message);
                        } else if (Objects.equals("ifMainPerson", key)) {
                        	String s = map.get(key).toString();
                        	String message = "";
                        	if (Objects.equals(s, YesOrNo.YES.getValue())) {
                        		message = YesOrNo.YES.getName();
                        	} else if (Objects.equals(s, YesOrNo.NO.getValue())) {
                        		message = YesOrNo.NO.getName();
                        	} else {
                        		message = s;
                        	}
                        	objects.add(message);
...
//        按库存组织、小类、职责、是否主责为Y，如果有重复行，把维护的早的删掉，保留最后维护的一条,只处理是否主责为Y的数据
     ...
     */
    @Override
    public List<DivisionCategory> listDivisionPurchaseStrategyMainPerson(List<CompanyDemotionOrg> companyDemotionOrgs) {
        if (CollectionUtils.isEmpty(companyDemotionOrgs)) {
            return Collections.EMPTY_LIST;
        }
        List<DivisionCategory> list = new ArrayList<>();

        List<Long> categoryIds = companyDemotionOrgs.stream().map(CompanyDemotionOrg::getCategoryId).distinct().collect(Collectors.toList());
        List<Long> orgIds = companyDemotionOrgs.stream().map(CompanyDemotionOrg::getOrgId).distinct().collect(Collectors.toList());
        // 根据数据数据过滤一部分数据出来
        List<DivisionCategory> filteredList = this.list(Wrappers.lambdaQuery(DivisionCategory.class)
                .eq(DivisionCategory::getDuty, DUTY.Purchase_Strategy.name())
        ...
        Map<Long, Map<Long, List<DivisionCategory>>> map = filteredList.stream().collect(Collectors.groupingBy(DivisionCategory::getCategoryId,
                Collectors.groupingBy(DivisionCategory::getOrgId)));

        companyDemotionOrgs.forEach(companyDemotionOrg -> {
            if (map.containsKey(companyDemotionOrg.getCategoryId())) {
                Map<Long, List<DivisionCategory>> subMap = map.get(companyDemotionOrg.getCategoryId());
                if (subMap.containsKey(subMap.get(companyDemotionOrg.getOrgId()))) {
                    List<DivisionCategory> subList = subMap.get(companyDemotionOrg.getOrgId());
