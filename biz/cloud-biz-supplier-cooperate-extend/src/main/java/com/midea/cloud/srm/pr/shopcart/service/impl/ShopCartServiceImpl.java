...
public class ShopCartServiceImpl extends ServiceImpl<ShopCartMapper, ShopCart> implements IShopCartService {
	...
	public void ceeaAddToShopCart(MaterialQueryDTO material) {
		...
		shopCart.setCategoryCode(material.getCategoryCode());
			...
			for(ShopCartImportModelDto shopCartImportModelDto:shopCartImportModelDtos) {
				...
				if(lineFlag){
					...
					if ("Y".equals(shopCart.getIfCatalog())) {
						...
						System.out.println(priceLibrary.getItemDesc());
						System.out.println(shopCart.getMaterialName());
						System.out.println(Objects.equals(priceLibrary.getItemDesc(),shopCart.getMaterialName()));
						System.out.println(StringUtils.equals(priceLibrary.getItemDesc(),shopCart.getMaterialName()));
						if(null != priceLibrary && Objects.equals(priceLibrary.getItemDesc(),shopCart.getMaterialName())){
							// 将价格、合同、币种写入购物车
...
//							errorMsg.append("该物料不是目录化物料; ");
							errorMsg.append("请检查物料名称与价格库中物料名称是否一致（注意符号中英文是否一致）");
						}
					}
				}
