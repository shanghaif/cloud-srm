...
//        FSSCResult fsscResult = ifsscReqService.applyFreeze(bgtCheckReqParamDto);
     ...
     * excel导入采购需求行
     */
    @PostMapping("/excelImportNew")
    public Map<String,Object> excelImport(Long orgId,Long organizationId,Long categoryId,
    		String categoryCode,String categoryName,String orgCode,String orgName,String organizationCode,String organizationName,@RequestParam("file") MultipartFile file , Fileupload fileupload) throws Exception {
        Assert.notNull(file, "文件上传失败");
        Assert.notNull(orgId, "请选择业务实体");
        Assert.notNull(organizationId, "请选择库存组织");
        Assert.notNull(categoryId, "请选择品类");
        RequirementHead requirementHead = new RequirementHead();
        requirementHead.setOrgId(orgId);
        requirementHead.setOrgCode(orgCode);
        requirementHead.setOrgName(orgName);
        requirementHead.setOrganizationId(organizationId);
        requirementHead.setOrganizationCode(organizationCode);
        requirementHead.setOrganizationName(organizationName);
        requirementHead.setCategoryId(categoryId);
        requirementHead.setCategoryCode(categoryCode);
        requirementHead.setCategoryName(categoryName);
        return iRequirementLineService.importExcelNew(requirementHead, file , fileupload );
    }
     ...
     * @return
     */
    @PostMapping("/listPageRequirementLineChart")
    public PageInfo<RequirementLine> listPageRequirementChart(@RequestBody RequirementHeadQueryDTO requirementHeadQueryDTO){
        return iRequirementLineService.listPageRequirementChart(requirementHeadQueryDTO);
    }
