...
public class ShopCartController extends BaseController {
	...
	private void shoppingCartViewLimit(QueryWrapper<ShopCart> wrapper){
		...
		if(!showAll && showYourself){
			wrapper.and(shopCartQueryWrapper -> {
				shopCartQueryWrapper.eq("SUMMARY_USER_ID" , user.getUserId())
						.or()
						.eq("NOTICE_EMP_NO" , user.getUsername()).or()
						.eq("CREATED_BY" , user.getUsername());
			});
		}
	}
