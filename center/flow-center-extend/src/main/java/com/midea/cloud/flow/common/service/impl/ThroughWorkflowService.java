package com.midea.cloud.flow.common.service.impl;
...
@Service
public class ThroughWorkflowService implements IWorkflowService {
	...
	@Override
	public String submit(Long formInstanceId, String type) throws Exception {
		if (null != type && null != formInstanceId ) {
			if (StringUtils.equals(type, TempIdToModuleEnum.REQUIREMENT.getValue())) {
				doRequirement(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.ORDER.getValue())) {
				doOrder(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.CONTRACT.getValue())) {
				doContract(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.REVIEW.getValue())) {
				doReview(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.SUPPLIERAUTH.getValue())) {
				doSupplierauth(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.CHANGE.getValue())) {
				doChange(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.IMPORT.getValue())) {
				doImport(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.ACCEPT.getValue())) {
				doAccept(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.COMPANYINFO.getValue())) {
				doCompanyinfo(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.VENDORASSES.getValue())) {
				docVendorasses(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.FORCOMPARISON.getValue())) {
				doBiding(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.BIDING.getValue())) {
				docForComparison(formInstanceId);
			} else if (StringUtils.equals(type, TempIdToModuleEnum.APPROVAL.getValue())) {
				doApproval(formInstanceId);
			} else {
				Assert.isTrue(false, "ES_RETURN_ID参数模板ID：" + type + "找不到");
			}
		}
		return String.valueOf(formInstanceId);
	}
